...
void RegionalElection::saveToFile() {
    std::ofstream file("data/elections/regional_" + getElectionId() + ".txt");
    if (file.is_open()) {
...
void RegionalElection::loadFromFile(std::string eId) {
    std::ifstream file("data/elections/regional_" + eId + ".txt");
    if (file.is_open()) {
