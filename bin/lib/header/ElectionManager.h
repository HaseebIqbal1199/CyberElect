...
public:
    ElectionManager();
    ~ElectionManager();
    bool createElection(std::string type, std::string name, std::string start, std::string end, std::string region = "", int regions = 0);
    void displayActiveElections();
    void displayAllElections();    Election* getElection(std::string electionId);
    ...
    bool updateElection(Election* election); // Public method to update an election
